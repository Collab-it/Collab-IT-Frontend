...
class Yjs extends Component {
  componentDidMount() {
    ...
    console.log(
      "Yjs - componentDidMount - this.props.showRoom is: ",
      this.props.showRoom
    );
  }
  ...
  render() {
    ...
    console.log(
      "Yjs -->>> connection.connected in render is: ",
    ...
    if (this.props.connectionExists === true) {
      Y({
        ...
        if (that.props.texteditorIsSummernote === true) {
          console.warn(
            "Yjs ---->>> this Yjs is being rendered by a Summernote text editor!!!"
          );
          console.warn(
            "Yjs --->>>> that.props.showRoom is: ",
            that.props.showRoom
          );
          // bind the textarea to a shared text element
          ...
          y.share.textarea.insert(
            0,
            `initial text for ${that.props.showRoom} here...`
        ...
        if (that.props.connectionExists === false) {
          console.log(
            "y - if connectionExists is false, destroy connection..."
          );
          y.destroy();
