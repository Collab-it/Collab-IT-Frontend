...
class TextEdit extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    if (this.props.connectionExists === nextProps.connectionExists) {
      //these two should always be equal...
      console.warn(
        "TextEdit - (this.props.connectionExists === nextProps.connectionExists), so returning FALSE in shouldComponentUpdate... AKA no re-render..."
      );
    ...
    return (
      <div className="TextEdit-style">
        ...
        {this.props.connectionExists === true ? (
          ...
          <h4>Textarea here when connection exists.</h4>
        )}
      </div>
    );
  }
